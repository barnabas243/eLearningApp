...
# Retrieve the chat room
    ...
    is_enrolled = Enrollment.is_student_enrolled(request.user, chat_room.course)
    is_teacher = Course.objects.filter(teacher=request.user).exists()

    if is_enrolled or is_teacher:
        # Get users in the course
        users = User.objects.filter(enrollment__course=chat_room.course)
